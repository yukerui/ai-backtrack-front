...
function ChecklistItem({
	...
	return (
		<label
			className={cn(
				"flex cursor-pointer items-start gap-3 rounded-xl border px-3 py-2 text-sm transition-colors",
				checked
...
export function BotFatherConsole({
	...
	const lastSetupCurrentStep = lastSetupWorkflow.currentStep;
	const selectedCurrentStep = selectedWorkflow?.currentStep || null;
						...
						<div className="grid gap-4 border-t px-4 pb-4 pt-4">
							...
							<div className="grid gap-3 sm:grid-cols-2">
								<ChecklistItem
									...
									id={inSheet ? "sheet-start" : "start"}
									label={isEditing ? "保存后尝试启动" : "创建后立即启动"}
					...
					{showDraftWorkbench ? (
											...
											>
												<div className="grid gap-3">
													<ChecklistItem
														...
														id="prepared-credentials"
														label="已复制 App ID / App Secret，并导入权限"
														onCheckedChange={(checked) => {
															updateCreatePreparationChecklist(
																"preparedCredentials",
																checked,
						...
						>
							...
							<CardContent className="space-y-4 text-sm">
								...
								<AnimatePresence initial={false} mode="popLayout">
									{lastSetupCurrentStep ? (
										<motion.div
											animate={{ opacity: 1, y: 0 }}
											exit={{ opacity: 0, y: -8 }}
											initial={{ opacity: 0, y: 12 }}
											key={lastSetupCurrentStep.key}
											layout
										>
											{lastSetupCurrentStep.key === "ownerPairing" ? (
												...
												<StepCard
													...
													step={
														...
														"configuredLongConnection"
															? 4
															: lastSetupCurrentStep.key === "addedMessageEvent"
																? 5
																: 6
													}
													title={lastSetupCurrentStep.title}
												>
													...
													<ChecklistItem
														...
														id={`last-setup-${lastSetupBotSlug || "draft"}-${lastSetupCurrentStep.key}`}
														label="我已完成这一步"
														onCheckedChange={(checked) => {
															if (!lastSetupBotSlug) {
																return;
															...
															{selectedCurrentStep.key === "ownerPairing" ? (
																...
																>
																	...
																	<ChecklistItem
																		...
																		id={`selected-step-${selectedBot.bot_slug}-${selectedCurrentStep.key}`}
																		label="我已完成这一步"
																		onCheckedChange={(checked) => {
																			updateChannelSetupChecklist(
																				selectedBot.bot_slug,
