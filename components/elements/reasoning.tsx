...
export const ReasoningTrigger = memo(
  ({
    ...
    return (
      <CollapsibleTrigger
        className={cn(
          "flex items-center gap-1 rounded-md px-1.5 py-0.5 text-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground",
          className
        )}
        {...props}
      >
  ...
  ({ className, children, ...props }: ReasoningContentProps) => (
    ...
    >
      <div className="max-h-48 overflow-y-auto rounded-md border border-border/50 bg-muted/30 p-2.5">
        <Response className="grid gap-1 text-[11px] **:text-[11px] [&_li]:my-0 [&_ol]:my-1 [&_p]:my-0 [&_ul]:my-1 whitespace-pre-wrap">
          {children}
        </Response>
      </div>
    </CollapsibleContent>
